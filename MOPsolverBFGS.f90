subroutine BFGS(n,m,x,l,u,strconvex,scaleF,checkder,outiter,time,nfev,ngev,theta,inform)
	...
	real(kind=8) :: g(n),xprev(n),JFprev(m,n),phi0(m),gphi0(m),fend(m)
  ...
  ! March of 2022.
	...
	Main_loop: do
		...
		if ( outiter == 0 ) then
			...
			call BFGSupdate(n,m,x,xprev,JF,JFprev,strconvex,theta,B)
		end if
