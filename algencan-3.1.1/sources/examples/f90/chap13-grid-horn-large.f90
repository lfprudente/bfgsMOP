...
program algencanma
  ...
  real(kind=8) :: cnorm,efacc,efstain,eoacc,eostain,epsfeas,epsopt, &
       f,nlpsupn,snorm,angle,posx,posy,a,b,c,step,gamma
  ...
  ! Set (bounds of the) points in the border
  ...
  do i = 1,nabs
     ...
     b = - 36.0d0 * a
     c = - 160.0d0 * a
     posx = 10.0d0 + 30.0d0 * ( dble(i) - 1 ) / ( nabs - 1 )
     posy = a * posx ** 2 + b * posx + c
  ...
  do j = 1,nord
     posx = 10.0d0
     posy = 10.0d0 + 10.0d0 * ( dble(j) - 1 ) / ( nord - 1 )
     l(II(1,j,'x'))    = posx
     l(II(1,j,'y'))    = posy 
     u(II(1,j,'x'))    = l(II(1,j,'x'))
     u(II(1,j,'y'))    = l(II(1,j,'y'))

     angle = - acos( 3.75d0 / 6.25d0 ) + 2.0d0 * acos( 3.75d0 / 6.25d0 ) * ( ( dble(j) - 1 ) / ( nabs - 1 ) )
     posx = 25.0d0 + 25.0d0 * cos( angle )
     posy = 20.0d0 + 25.0d0 * sin( angle )
     l(II(nabs,j,'x')) = posx
     l(II(nabs,j,'y')) = posy
  ...
  do j = 1,nord
     x(II(1,j,'x')) = l(II(1,j,'x'))
     x(II(1,j,'y')) = l(II(1,j,'y'))

     x(II(nabs,j,'x')) = l(II(nabs,j,'x'))
     x(II(nabs,j,'y')) = l(II(nabs,j,'y'))
  end do
...
subroutine myevalfc(n,x,f,m,c,flag)
  ...
  !write(*,*) 'valor de f1 = ',f1,' valor de f2 = ',f2,' valor de f = ',f
  ...
  do i = 1,nabs
     do j = 1,nord
        if ( i .lt. nabs ) then
           ...
           yy2 = x(II(i+1,j,'y'))
           inside1 = .false.
           if ( xx1 .ge. 12.0d0 .and. xx1 .le. 23.0d0 .and. &
                yy1 .ge. 10.0d0 .and. yy1 .le. 19.0d0 ) then
              inside1 = .true.
           end if
           inside2 = .false.
           if ( xx2 .ge. 12.0d0 .and. xx2 .le. 23.0d0 .and. &
                yy2 .ge. 10.0d0 .and. yy2 .le. 19.0d0 ) then
              inside2 = .true.
           end if
...
20 format('drawdot (',f20.10,'u,',f20.10,'u) withpen pencircle scaled 0.01;')
21 format('dotlabel ( btex $(',i2,',',i2,')$ etex, (',f20.10,'u,',f20.10,'u)) withpen pencircle scaled 0.01;')
30 format('draw (',f20.10,'u,',f20.10,'u)--(',f20.10,'u,',f20.10,'u) withpen pencircle scaled 0.005 withcolor .7white;')
