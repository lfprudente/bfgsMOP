...
program algencanma
  ...
  do i = 1,nabs
     ...
     b = - 3.6d0 * a
     c =   1.0d0 + 2.6d0 * a
     posx = 1.0d0 + 3.0d0 * ( dble(i) - 1 ) / ( nabs - 1 )
  ...
  do j = 1,nord
     posx = 1.0d0
     posy = 1.0d0 + 1.0d0 * ( dble(j) - 1 ) / ( nord - 1 )
     l(II(1,j,'x'))    = posx
     l(II(1,j,'y'))    = posy 
     ...
     posx = 2.5d0 + 2.5d0 * cos( angle )
     posy = 2.0d0 + 2.5d0 * sin( angle )
     l(II(nabs,j,'x')) = posx
     l(II(nabs,j,'y')) = posy
...
subroutine drawsol(n,x)
  ...
  ! LOCAL SCALARS
  integer :: i,j
  real(kind=8) :: scale,maxx,maxy,minx,miny
